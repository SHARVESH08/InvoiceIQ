...
export function CustomerEditForm({ customer }: CustomerEditFormProps) {
  const [gstinStateName, setGstinStateName] = useState<string | null>(() => {
    if (customer.gstin && customer.state_code) {
      return GSTIN_STATE_CODES[customer.state_code] ?? null
    }
    return null
  })
  ...
  const form = useForm<CustomerInput>({
    ...
    defaultValues: {
      name: customer.name,
      customer_type: (customer.customer_type as 'b2b' | 'b2c') ?? 'b2c',
      gstin: customer.gstin ?? '',
      phone: customer.phone ?? '',
      email: customer.email ?? '',
      state_code: customer.state_code ?? '',
      credit_limit: Number(customer.credit_limit),
      // Map billing_address jsonb back to flat form fields
  ...
  async function onSubmit(values: CustomerInput) {
    ...
    // On success: updateCustomer redirects server-side — client code below never runs
  }
